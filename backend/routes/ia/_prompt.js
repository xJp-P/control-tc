...
function construirPrompt(movimientos, textoExtracto, bancoDoc, contextoUsuario, cruce, estrategia) {
  ...
  const systemArr = [
    ...
    '4h. REVERSO vs ANULACION: NO son lo mismo y la accion es distinta. Un movimiento NEGATIVO que cancela una compra puede ser una de dos cosas, y se distinguen por el NUMERO DE AUTORIZACION y la FECHA. (a) ANULACION: el negativo lleva la MISMA autorizacion que el cargo y la MISMA fecha. El banco anulo la operacion, asi que NUNCA entro a la facturacion y NO esta sumada en la cuota de transacciones -compruebalo: si al sumar los cargos del periodo te SOBRA justo ese monto, es una anulacion-. En la app hay que ANULAR: tipo="compra_anulada", operacion="anular_plan", parametros { compra_id }, con impacto_pago_minimo NEGATIVO por lo que la app cobre de ella este ciclo (la cuota si es diferida, el valor completo si es de contado), porque es dinero que la app cobra y el banco no. (b) REVERSO o devolucion: el negativo lleva OTRA autorizacion o es de otra fecha. El banco SI facturo la compra y acredito la devolucion aparte, normalmente en "Pagos/abonos": la compra NO sobra en el ciclo, se usa "reversar_compra" y su impacto sobre el pago minimo es 0 (el cargo sigue contando; lo que cambia es el saldo). Si el extracto no imprime autorizaciones, guiate por la fecha: mismo dia y mismo monto exacto que el cargo -> anulacion; dias despues -> reverso. Ante la duda reporta reverso y explica por que, que es la opcion que NO borra proyecciones.',
    ...
    JSON.stringify({
      ...
      discrepancias: [{ tipo: 'compra_omitida|compra_no_facturada|monto_erroneo|clasificacion_incorrecta|cuota_reprogramada|diferida_omitida|compra_anulada|otro', descripcion: 'string', valor_extracto: 0, valor_app: 0, compra_id: null, impacto_pago_minimo: 0, severidad: 'alta|media|baja', accion_sugerida: { operacion: 'crear_compra|eliminar_compra|editar_valor|convertir_a_diferida|reprogramar_cuotas|crear_diferida_omitida|mover_ciclo|reversar_compra|anular_plan|ninguna', parametros: {} } }]
    })
  ];
